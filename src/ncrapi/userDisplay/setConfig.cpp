    ...
    for (auto &it : sysData->obj) //遍历项目名字
    {
        ...
        for (auto &it : _tempData[it->showName()].items())
        {
            if (it.key() != "PID参数")
                ...
                {
                    key.push_back(lv_label_create(tabs, nullptr));    //KEY就一个
                    ...
                    {
                        /**value设置*/
                        lv_obj_t *values = lv_label_create(tabs, nullptr);     //新建子项目的val
                        lv_label_set_text(values, it2.value().dump().c_str()); //设置按钮的值
                        if (!it1.value().is_array())                           //如果他不是数组就创建key
                        {
                            /**key设置*/
                            key.push_back(lv_label_create(tabs, nullptr));    //新建子项目的key
                            ...
                            /**value设置*/
                            lv_obj_align(values, key.back(), LV_ALIGN_OUT_RIGHT_MID, 10, 0); //values的位置紧贴key的位置
                        }
                        else //是数组
                        {
                            /**value设置*/
                            if (pos == 0)
                                ...
                                lv_obj_align(values, updownBtn.back(), LV_ALIGN_OUT_RIGHT_MID, 10, 0); //values的位置紧贴key的位置
                        }

                        /**updownBtn设置*/
                        updownBtn.push_back(lv_btnm_create(tabs, nullptr)); //新建updownBtn
                        lv_btnm_set_map(updownBtn.back(), updown);
                        lv_obj_set_size(updownBtn.back(), 50, 25);

                        lv_obj_set_free_ptr(updownBtn.back(), values); //创建关键指针. btn关联values 这里主要为了实现刷新
                        lv_obj_set_free_ptr(values, &it2.value());     //创建关键指针. values关联key

                        lv_obj_align(updownBtn.back(), values, LV_ALIGN_OUT_RIGHT_MID, 10, 0); //设置updownbtn位置
                        lv_btnm_set_action(updownBtn.back(), upDownAction);                    //设置updown按钮动作
                        pos++;
                    };
                    if (lv_obj_get_x(updownBtn.back()) > 230) //key表示偶数换行 it.size()表示每行总共有2个数一个KEY 一个VAL
                    {
                        posX = 0;
                        posY += 35;
                    }
