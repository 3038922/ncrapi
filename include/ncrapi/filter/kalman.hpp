...
class KalmanFuse : public KalmanFilter<double>
{

  public:
    KalmanFuse(const double Q, const double R) : KalmanFilter(Q, R) {}
    /**
 * 以sensor1 为基准,sensor2为辅助 
 * @param sensor1 
 * @param sensor2 
 * @return double 
 */
    double filter(const double &sensor1, const double &sensor2)
    {
        _sensor2Now = sensor2;
  ...
  private:
    double _sensor2Now = 0;
    double _sensor2Last = 0;
};
} // namespace ncrapi